...
function calculateMixedFractions(whole1: number, num1: number, den1: number, whole2: number, num2: number, den2: number, operation: Operation): string {
  let steps: string[] = [];
  ...
  steps.push(`Step 2: Perform the ${operation === '×' ? 'multiplication' : operation === '÷' ? 'division' : 'operation'}`);

  switch (operation) {
    case '+':
    case '-':
      const commonDen = lcm(impDen1, impDen2);
      const newNum1 = impNum1 * (commonDen / impDen1);
      const newNum2 = impNum2 * (commonDen / impDen2);
      resultNum = operation === '+' ? newNum1 + newNum2 : newNum1 - newNum2;
      resultDen = commonDen;
      steps.push(`Find the common denominator: LCM(${impDen1}, ${impDen2}) = ${commonDen}`);
      steps.push(`Adjust the numerators:`);
      steps.push(`\\frac{${impNum1}}{${impDen1}} = \\frac{${newNum1}}{${commonDen}}`);
      steps.push(`\\frac{${impNum2}}{${impDen2}} = \\frac{${newNum2}}{${commonDen}}`);
      steps.push(`\\frac{${newNum1}}{${commonDen}} ${operation} \\frac{${newNum2}}{${commonDen}} = \\frac{${resultNum}}{${resultDen}}`);
      break;
    case '×':
  ...
  if (mixedWhole !== 0) {
    steps.push(`Step 4: Convert to mixed number`);
...
export function MixedFractionsCalculator() {
  ...
  const handleCalculate = () => {
    try {
      const calculationSteps = calculateMixedFractions(
        parseInt(whole1) || 0,
        parseInt(num1) || 0,
        parseInt(den1) || 1,
        parseInt(whole2) || 0,
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
      <CardHeader>
        <CardTitle className="text-2xl font-bold">Mixed Fractions Calculator</CardTitle>
      </CardHeader>
      <CardContent>
